...
if (!fs.existsSync('data/cache/dump')) {
    const master: DiskStore = new DiskStore('data/cache/main_file_cache.dat2', 'data/cache/main_file_cache.idx255', 255);
    ...
    for (let i: number = 0; i < Js5Archive.values.length; i++) {
        ...
        const datName: string = type.name === 'audiostreams' ? 'main_file_cache.dat2m' : 'main_file_cache.dat2';
        const store: DiskStore = new DiskStore(`data/cache/${datName}`, `data/cache/main_file_cache.idx${type.id}`, type.id);
